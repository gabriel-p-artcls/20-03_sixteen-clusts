...
reddenings, masses, distances and ages of sixteen faint and mostly unstudied 
...
performed. The photometric observations were reduced and then analyzed in an
...
{
Various zero point corrections for Gaia DR2 parallax data reported in recent
...
\citet{2002A&A...389..871D} \textbf{(see bibtex entry in .tex file, just below this line)}, 
...
%}
previously unstudied, located in a poorly known Galactic sector, at approximately
...
combining  \textit{UBVI} CCD data with space based astrometry (parallax,
and proper motions) and photometry. Second, since distance is the main derived parameter for
...
In following studies we aim at aim at investigating the structure of the Galactic disk in
this region. Traces of the Perseus Arm coming from of the third Galactic quadrant  would be expected, 
despite this arm being only prominent in the second quadrant. However, one must keep in mind 
...
Table \ref{tab:clust_list} lists the equatorial coordinates ($\alpha$, $\delta$) and
galactic coordinates (\textit{l}, \textit{b}) of the 16 cluster fields studied here, 
ordered by increasing right ascension $\alpha$. Equatorial coordinates
...
\begin{table}[ht]
    ...
    Note: van den Bergh-Hagen clusters are
...
first 30 degrees of the fourth Galactic quadrant, from latitudes $l\sim$273$^\circ$ to
$l\sim$300$^\circ$, encompassing the region around the Carina OB association and the
...
which indicates a good agreement. In Fig \ref{fig:gaia_transf} we show as
...
The starting point is that the region occupied by a real cluster
...
The second block assigns membership probabilities, an often disregarded
...
must be done. Ideally, we look for firm evidence of the presence of a
cluster sequence at some evolutionary stage. \texttt{ASteCA} employs a
...
figures. The first shows the photometric diagrams built with
all the stars observed in the each targeted region.
The second figure represents the spatial analysis carried out by
\texttt{ASteCA}. This is: results from the search of a stellar overdensity, the
mean value for the stellar field 
\textbf{**Mean value of what? I think we want to say  "the central coordinates of the cluster"**}, 
the respective King profile\footnote{We
...
fitting of a synthetic cluster yields the following results:
...
This is an object in the Crux constellation, placed slightly south of the Crux geometric
...
This analysis points to a required bias correction to Gaia parallaxes
...
comparing with parallax distances obtained with no correction applied, is around
$\sim+0.024$ mas, thus closest to the +0.029 mas value given by Lindegren et
al. Certainly, analysis of a more extended sample of clusters is needed for arriving to
 conclusive results and establish the detailed relation between distances
from photometry and DR2 parallaxes. The results of the exercise presented in this section are included
...
We have analyzed the fields of sixteen catalogued
...
An interesting result provided by the \texttt{ASteCA} analysis is the
metal content $z$ for each cluster included in the second column of Table 
\ref{tab:final_tab}. If we average the values found for all the clusters
the metal content is $z=0.0154$ with a standard deviation $\sigma=0.007$.
The result is well in agreement with the assumption that the typical Milky Way open
cluster has solar metallicity ($z=0.0154$, \textbf{**citet reference**})
...
reservation. Two other objects, TR 13 and vdBH 92, are young with ages close to and
...
that is comprised within the estimated maximum and minimum distance for Carina. vdBH 106 
(3 Gyr) and vdBH 85 (7.5 Gyr) are the oldest objects found in our search and
...
With respect to photometric versus parallax distances, we can conclude that by adding
